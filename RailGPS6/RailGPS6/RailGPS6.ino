//Railtour GPS data gatherer VI
//Vitezslav Dostal | started 20.06.2022
//Hardware required: T-Call V1.4 ESP32 featuring SIM800L & AT24C128 & UbloxNeo 
...
#ifndef PRIMARY
#define PRIMARY 52                                         //Number of measures sent (out of all CYCLES)
#endif
...
#ifndef TERTIARY
#define TERTIARY 41                                        //Number of measures sent since the last successful cycle
#endif
...
#ifndef MODULO
#define MODULO 8                                           //Number of measures followed by data sending
#endif
...
void gprs() {
  console.println();
  load((char*)&c0);  modem.println(buffer); myDelay(500);
...
void sendCommand()
{
  ...
  if (modify)
  {
    load((char*)&server); console.print(buffer); modem.print(buffer);
    load((char*)&c36); console.print(buffer); modem.print(buffer);
  }
...
void receiveCommand()
{
  ...
  while (!modem.available() && millis() - stamp < limit) {wdt_reset();};
...
void trasmit()
{
  load((char*)&c16); modem.print(buffer); //POST /script/measure_add_gps2.php HTTP/1.1
  load((char*)&c17); modem.print(buffer);
  load((char*)&c18); modem.print(buffer); //Host:
  load((char*)&server); modem.print(buffer); //SERVER
  load((char*)&c17); modem.print(buffer);
  load((char*)&c19); modem.print(buffer); //User-Agent: ArduinoSIM800
  modem.print(sensor);
  load((char*)&c17); modem.print(buffer);
  load((char*)&c20); modem.print(buffer); //Connection: close
  load((char*)&c17); modem.print(buffer);
  load((char*)&c21); modem.print(buffer); //Content-Type: application/x-www-form-urlencoded
  load((char*)&c17); modem.print(buffer);
  load((char*)&c22); modem.print(buffer); //Content-Length:
  modem.print(packets * PACKET + strlen(APIKEY) + strlen(sensor) + 4 + 2 + 4); // Content + Key + Sensor + Voltage + Flags + Delimiters
  load((char*)&c17); modem.print(buffer);
  load((char*)&c17); modem.print(buffer);

  load((char*)&key); modem.print(buffer); //KEY
  load((char*)&c23); modem.print(buffer); //Separator |
  modem.print(sensor);
  load((char*)&c23); modem.print(buffer); //Separator |
  modem.print(voltage);
  load((char*)&c23); modem.print(buffer); //Separator |
  modem.write(flags); BIT_CLEAR(flags, FLAGS_ALARM_NOW);
  modem.print(delim[1]);
  load((char*)&c23); modem.print(buffer); //Separator |
  console.print(buffer);
  ...
  console.println();
  load((char*)&c17);
  for (int i = 0; i < 2; i++) modem.print(buffer); //LINE
}
...
int transmitPacket(int i, int base, bool testrun, bool simulation)
{
  ...
  if (!simulation) {
    ...
    for (byte j = 0; j < PACKET; j++) modem.write(memory[j]);
    console.print(which);
    console.print(buffer);
  } else {
    packets++;
